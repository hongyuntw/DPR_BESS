...
class BiEncoderTrainer(object):
    ...
    def __init__(self, args):
        ...
        tensorizer, model, optimizer , augument_tensorizer = init_biencoder_components(
        ...
        self.augument_tensorizer = augument_tensorizer
        self.start_epoch = 0
        self.start_batch = 0
        ...
        print('load question scores...')
        with open('./data/retriever/question_score_mask32_b12_cn', 'rb') as handle:
            self.question_scores = pickle.load(handle)
    ...
    def validate_nll(self) -> float:
        ...
        for i, samples_batch in enumerate(data_iterator.iterate_data()):
            ...
            loss, correct_cnt = _do_biencoder_fwd_pass(
                self.biencoder, biencoder_input, self.tensorizer, self.augument_tensorizer, args
            )
    ...
    def validate_average_rank(self) -> float:
        ...
        for i, samples_batch in enumerate(data_iterator.iterate_data()):
            ...
            for j, batch_start in enumerate(range(0, bsz, sub_batch_size)):
                ...
                q_attn_mask = self.augument_tensorizer.get_attn_mask(q_ids)
    ...
    def _train_epoch(
        ...
        for i, samples_batch in enumerate(
            ...
            loss, correct_cnt = _do_biencoder_fwd_pass(
                self.biencoder, biencoder_batch, self.tensorizer, self.augument_tensorizer, args
            )
...
def _do_biencoder_fwd_pass(
    ...
    q_attn_mask = augument_tensorizer.get_attn_mask(input.question_ids)
